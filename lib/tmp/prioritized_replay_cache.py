...
class PrioritizedReplayCache(object):
    ...
    def sample_batch(self, batch_size):
        ...
        if self.count > batch_size * 10:
            sample_size = batch_size * 10
            batch = random.sample(self.buffer, sample_size)
            batch = np.sort(batch, axis=0)

            split = (batch_size - 1) / 2
            batch = np.concatenate((batch[0:split], batch[(-split-1):-1]), axis=0)
            print(sample_size)
        else:
            batch = random.sample(self.buffer, sample_size)
            print(sample_size)
