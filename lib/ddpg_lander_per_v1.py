...
# Lessons learned
# ==================================
# FAILED.  KEEP ON AVOIDING LANDING AND LEARNED A BIT HERKY/JERKY.
# TRY AIDEN'S TWO SIDED MEMORY IDEA.  NEXT
# ALSO TRY ADDING IN MORE NOISE SO LOW ONES ARE PICKED MORE FREQUENTLY
...
E = 0.01 # forces adding some error.  not really needed since put a np.maximun(error, 1) to ensure at least 1 appears
# A = 0.6
# A = .8
ERROR_SCALAR = 100
    ...
    for i in range(MAX_EPISODES):
        ...
        for j in range(MAX_EP_STEPS):
            ...
            #     print("sample", replay_buffer.sample(MINIBATCH_SIZE))
                ...
                errors_batch = np.array([ o[0] for o in batch ])
            ...
            #     replay_buffer.sample(MINIBATCH_SIZE)
                ...
                # print("y_i", np.reshape(np.amax(y_i), (MINIBATCH_SIZE, 1)))
                # print("predicted_q_value", predicted_q_value)
                updated_errors = abs(np.reshape(y_i, (MINIBATCH_SIZE, 1)) - predicted_q_value)
                # print(updated_errors)
                #update errors
                # abs(np.reshape(y_i, (MINIBATCH_SIZE, 1)) - predicted_q_value)
                for index in range(len(errors_batch)):
                    idx = errors_batch[index]
                    replay_buffer.update(idx,  np.amax(updated_errors[index][0]))
